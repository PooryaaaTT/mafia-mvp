...
io.on("connection",s=>{
 ...
 s.on("game:start",({code:c},cb)=>{const r=rooms.get(String(c||"").toUpperCase());if(!r)return cb?.({ok:false,error:"اتاق پیدا نشد"});const me=r.players.find(p=>p.id===s.id);if(!me?.host)return cb?.({ok:false,error:"فقط میزبان"});if(r.players.length<4)return cb?.({ok:false,error:"حداقل ۴ بازیکن لازم است"});let roles=["mafia","doctor","detective",...Array(Math.max(1,r.players.length-3)).fill("citizen")].slice(0,r.players.length).sort(()=>Math.random()-.5);r.players.forEach((p,i)=>{p.role=roles[i];p.alive=true;});r.started=true;r.phase="night";r.players.forEach(p=>io.to(p.id).emit("role:assigned",{role:p.role}));emit(r.code);cb?.({ok:true});});
